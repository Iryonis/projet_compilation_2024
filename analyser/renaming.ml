...
let rename_arg arg name_counter = 
   match arg with 
   | Argument (name, typ, annotation) -> 
      match Environment.get name_counter name with
      |None -> 
         Environment.add name_counter name 0;
         (Argument (name, typ, annotation), name_counter)
      |Some i -> Environment.modify name_counter name (i+1);
         (Argument (name ^ "#" ^ (string_of_int (i + 1)), typ, annotation), name_counter)

let rec rename_list_args args name_counter = 
   match args with 
   | [] -> []
   | arg::q -> 
      let (arg, name_counter) = rename_arg arg name_counter in
      arg:: ( rename_list_args q name_counter)
...
let rec rename_expr expr env =
   ...
   |Color(expr1, expr2, expr3, annotation) ->
      (Color(rename_expr expr1 env, rename_expr expr2 env, rename_expr expr3 env, annotation))
   ...
   |Variable(id, annotation) ->(
      ...
      |None ->  Environment.add env id 0; (Variable(id, annotation))
   ...
   |Binary_operator(op, expr1, expr2, annotation) -> 
      (Binary_operator(op, rename_expr expr1 env, rename_expr expr2 env, annotation))
...
let rec rename_statement statement env = 
   ...
   |Declaration(name, typ_expr, annotation) ->(
      ...
      |None -> 
         Environment.add env name 0; (Declaration(name, typ_expr, annotation))
      |Some i -> Environment.modify env name (i+1); (Declaration(name ^ "#" ^ (string_of_int (i + 1)), typ_expr, annotation)))
   |Block(statement_list, annotation) -> 
      let env_copy = Environment.copy env in (Block((List.map(fun x -> rename_statement x env_copy) statement_list), annotation))
   |IfThenElse(test, th, el, annotation) -> 
      let th_copy = Environment.copy env in let el_copy = Environment.copy env in (IfThenElse(rename_expr test env, rename_statement th th_copy, rename_statement el el_copy, annotation))
   |For(id, expr1, expr2, expr3, statement, annotation) -> (
      let statement_copy = Environment.copy env in match Environment.get env id with
      |None -> 
         Environment.add statement_copy id 0; (For(id, rename_expr expr1 env, rename_expr expr2 env, rename_expr expr3 env, rename_statement statement statement_copy, annotation))
      |Some i -> Environment.modify statement_copy id (i+1); (For(id ^ "#" ^ (string_of_int (i + 1)), rename_expr expr1 env, rename_expr expr2 env, rename_expr expr3 env, rename_statement statement statement_copy, annotation)))

   |Foreach(id, test, body,annotation) ->(
      let body_env = Environment.copy env in match Environment.get env id with
      |None -> Environment.add body_env id 0;  (Foreach(id, rename_expr test env, rename_statement body body_env, annotation))
      |Some i -> Environment.modify body_env id (i+1); (Foreach(id ^ "#" ^ (string_of_int (i + 1)), rename_expr test env, rename_statement body body_env, annotation)))
   |Draw_pixel(expr, annotation) -> Draw_pixel(rename_expr expr env, annotation)
   |Nop -> Nop
